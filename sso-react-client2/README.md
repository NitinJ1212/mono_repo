...
POST http://localhost:3000/admin/clients
{
  ...
  "redirect_uris": ["http://localhost:4009/auth/callback"],
...
```sql
UPDATE clients
SET logout_uri = 'http://localhost:4009/backchannel-logout'
...
REDIRECT_URI=http://localhost:4009/auth/callback
...
8. SSO redirects browser → GET http://localhost:4009/auth/callback?code=...&state=...
9. Backend:
...
2. SSO calls POST http://localhost:4009/backchannel-logout { user_id }
